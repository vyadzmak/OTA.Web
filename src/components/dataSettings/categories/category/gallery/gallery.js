...
export default {
  ...
  computed: {
    ...mapGetters({item: 'productCategories/item', items: 'attachments/items'})
  },
  methods: {
    ...
    qDialogClose: async function (confirmed, data) {
      if (confirmed) {
        ...
        this.dialogData = await this.$store.dispatch('productCategories/updateItem', {item: updateItem, isUpdate: true})
