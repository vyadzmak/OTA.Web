...
export default {
  ...
  computed: {
    ...mapGetters({item: 'brandsCatalog/item', items: 'attachments/items'})
  },
  methods: {
    ...
    qDialogClose: async function (confirmed, data) {
      if (confirmed) {
        ...
        this.dialogData = await this.$store.dispatch('brandsCatalog/updateItem', {item: updateItem, isUpdate: true})
